...
func TestInfiniteLoopPreemption(t *testing.T) {
	ctx := context.Background()
	engine, err := gruntime.NewEngine(ctx)
	if err != nil {
		t.Fatalf("Failed to create engine: %v", err)
	...
	// MaxMemoryPages is critical here: it forces the engine to use a memory-limited
	// wazero runtime. Before the fix, this runtime was missing WithCloseOnContextDone(true),
	// meaning context cancellation would never propagate into Wasm execution.
	...
	// Cancel context before calling the proxy. The proxy checks ctx.Err() on entry.
	cancelledCtx, cancel := context.WithCancel(ctx)
	cancel() // immediately cancelled

	_, err = proxy.Parse(cancelledCtx, []byte("key: value"))
	if err == nil {
		t.Fatalf("Expected error from cancelled context, got nil")
	}

	// Verify that a call with a context that times out before completion also fails.
	// The large YAML payload maximises the chance of actually hitting the Wasm boundary.
	timedCtx, timedCancel := context.WithTimeout(ctx, 1*time.Nanosecond)
	defer timedCancel()
	// Give scheduler a moment to mark ctx as done
	time.Sleep(1 * time.Millisecond)

	_, err = proxy.Parse(timedCtx, []byte("key: value"))
	if err == nil {
